...
enum RETURNCODE
{
     ...
     /*
     Above we have 2x 96, dont ask me why but this is posix standard
     which is why we assign the return values instead of letting 
     the enum handle it :( 
    
     Also i am not sure if we should keep this as a enum or if we should be using them as a define makes more sense
     as with this implementation `return EPERM` will not work
    */
};