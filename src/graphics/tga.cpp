#include <graphics/tga.hpp>
namespace Graphics
{

unsigned int TGA::ParseTga(unsigned char *ptr, int size)
{
    unsigned int *data;
    ...
    switch(ptr[2]) {
        case 1:
            if(ptr[6]!=0 || ptr[4]!=0 || ptr[3]!=0 || (ptr[7]!=24 && ptr[7]!=32)) { mem_free(data); return NULL; }
            for(y=i=0; y<h; y++) {
                k = ((!o?h-y-1:y)*w);
                for(x=0; x<w; x++) {
            ...
            break;
        case 2:
            if(ptr[5]!=0 || ptr[6]!=0 || ptr[1]!=0 || (ptr[16]!=24 && ptr[16]!=32)) { mem_free(data); return NULL; }
            for(y=i=0; y<h; y++) {
