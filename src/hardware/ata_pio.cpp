...
uint8_t identify() {
    ...
    term_writeln("Waiting for status.\n");
    while(status & STAT_BSY) {
        uint32_t i = 0;
        while(1) {
            term_writeln_dec("Printing stuff ", i);
            i++;
        }
        for(i = 0; i < 0x0FFFFFFF; i++) {}
        term_writeln("Checking regstat.");
        status = inb(ATA_PRIMARY_COMM_REGSTAT);
    }
    
    if(status == 0) return 0;

    term_writeln("Status indicates presence of a drive. Polling while STAT_BSY... ");
    while(status & STAT_BSY) {
      term_writeln("\ninb(ATA_PRIMARY_COMM_REGSTAT);... ");
      status = inb(ATA_PRIMARY_COMM_REGSTAT);
    }
    ...
    term_writeln("Waiting for ERR or DRQ.\n");
    // Wait for ERR or DRQ
    ...
    term_writeln("Reading IDENTIFY structure.\n");
    //uint8_t *buff = kmalloc(40960, 0, NULL);
    uint8_t buff[256 * 2];
    insw(ATA_PRIMARY_DATA, buff, 256);
    term_writeln("Success. Disk is ready to go.\n");
    // We read it!
    return 1;
}
...
void ata_pio_read28(uint32_t LBA, uint8_t sectorcount, uint8_t *target) {
    ...
    for(i = 0; i < sectorcount; i++) {
        ...
        // Transfer the data!
        insw(ATA_PRIMARY_DATA, (unsigned char *)target, 256);
        target += 256;
    }
    ...
    for(i = 0; i < sectorcount; i++) {
        ...
        while(1) {
            ...
            if(status & STAT_DRQ) {
                ...
                debug_throw_panic("DISK SET ERROR STATUS!");
            }
        }
        // Transfer the data!
        outsw(ATA_PRIMARY_DATA, (unsigned char *)target, 256);
        target += 256;
    }
