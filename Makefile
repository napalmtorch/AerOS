...
qemu:
	qemu-system-i386 -m 256M -drive file=disks/disk.img,format=raw -cdrom 'AerOS.iso' -serial stdio -boot d -rtc base=$(shell date +"%y-%m-%yT-%r") -soundhw all -device e1000 -enable-kvm -cpu host -name "AerOS Development Copy"

qemu-kernel:
	qemu-system-i386 -m 256M -drive file=disks/disk.img,format=raw -kernel bin/kernel.bin -serial stdio -boot d -rtc base=$(shell date +"%y-%m-%yT-%r") -soundhw all -enable-kvm -cpu host -name "AerOS Development Copy"
